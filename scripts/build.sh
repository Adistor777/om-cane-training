...
# ---- 1. School-ID consistency guard (index.html vs supabase/schema.sql) ------
app_ids=$(grep -o "id:'sch_[a-z_]*'" index.html | grep -o "sch_[a-z_]*" | sort -u)
sql_ids=$(grep -o "('sch_[a-z_]*'" supabase/schema.sql | grep -o "sch_[a-z_]*" | sort -u)
[ -n "$app_ids" ] || fail "no sch_* school IDs found in index.html (seedSchools)"
[ -n "$sql_ids" ] || fail "no sch_* school IDs found in supabase/schema.sql seed"
if [ "$app_ids" != "$sql_ids" ]; then
  echo "--- index.html ---"; echo "$app_ids"
  echo "--- schema.sql ---"; echo "$sql_ids"
  fail "school IDs disagree between index.html and supabase/schema.sql"
fi
echo "OK  school IDs consistent ($(echo "$app_ids" | wc -l | tr -d ' ') schools)"

# ---- 2. JS parse check on the inline script -----------------------------------
node -e '
const fs = require("fs");
const html = fs.readFileSync("index.html", "utf8");
const m = html.match(/<script[^>]*>([\s\S]*?)<\/script>/g) || [];
const body = m.map(s => s.replace(/<\/?script[^>]*>/g, "")).join("\n");
try { new Function(body); console.log("OK  JS parse"); }
...
built="android/app/src/main/assets/public/index.html"
[ -f "$built" ] || fail "built assets not found at $built"
cmp -s www/index.html "$built" || fail "built index.html differs from www/ copy — sync did not land"
